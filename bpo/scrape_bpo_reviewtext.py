...
with open(outfile, mode = 'w', encoding = 'utf-8') as f:
    ...
    for seq, rec in recordsfromallpaths:
        ...
        f.write('=========================================================================\n')
        f.write('   '.join([str(seq), rec['RecordID'], rec['RecordTitle']]) + '\n')
        f.write('  '.join([rec['AlphaPubDate'], rec['SubjectTerms']]) + '\n')
        f.write(' '.join([rec['Title'], rec['Volume'], ':', rec['Issue'], rec['StartPage'], '-', rec['EndPage']]) + '\n')
        f.write('--------------------------------------------------------------------------\n')
