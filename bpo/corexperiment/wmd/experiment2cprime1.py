...
if not havedocs:
    ...
    book_vectors = get_vectors('bookvectors.tsv')
    review_vectors = get_vectors('reviewvectors.tsv')
...
with open('exp2cprime1.tsv', mode = 'w', encoding = 'utf-8') as f:
...
for cat, booklist in genrebooks.items():
    ...
    if len(pairstouse) > 150:
        pairstouse = random.sample(pairstouse, 150)
    ...
    for b1, b2 in pairstouse:
        ...
        with open('exp2cprime1.tsv', mode = 'a', encoding = 'utf-8') as f:
