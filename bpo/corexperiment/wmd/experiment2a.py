...
with open('exp2results.tsv', mode = 'w', encoding = 'utf-8') as f:
    f.write('category\tbook1\tbook2\tdate1\tdate2\trandombpo\tingroupbpo\tbpodiff\trandomhtrc\tingrouphtrc\thtrcdiff\tleninbpo\tleninhtrc\tlenrandbpo\tlenrandtrc\tn')
...
for k, v in books_in_cat.items():
    categories_to_use.append((len(v), k))

categories_to_use = sorted(categories_to_use, reverse = True)

for count, cat in categories_to_use:
    bookset = books_in_cat[cat]
    ...
    for b1, b2 in pairstouse:
        ...
        rand_bpo, rand_htrc, len_rand_bpo, len_rand_htrc = get_two_distances(randidx1, randidx2)

        if in_bpo < -100 or in_htrc < -100 or rand_bpo < -100 or rand_htrc < -100:
            continue

        with open('exp2results.tsv', mode = 'a', encoding = 'utf-8') as f:
