    ...
    if flag == 1:  # if the volume has an index, do the following steps
        ...
        for i in range(index_startpage, index_endpage):
            for j in range(len(pagelist[i])):
                #if(volume1987==1 and i>=2125 and i<=2170):
                #    print(pagelist[i][j])
        ...
        for j in range(linelength):  # begin to generate a list of headings within fiction
            ...
            if (volume1983_and_later==1): # deal with volumes after 1983 individually
                if((volume1983==1 and text[j] == "1642") or (volume1984==1 and text[j]=="1766") or (volume1985==1 and text[j]=="1816") or (volume1986==1 and text[j]=="1841") or (volume1987==1 and text[j]=="2130")):
                    for k in range(1,linelength-j):
                        if(text[j+k]=='Fiction'):
