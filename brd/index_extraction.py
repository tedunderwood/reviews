    ...
    if flag == 1:  # if the volume has an index, do the following steps
        ...
        for j in range(linelength):  # begin to generate a list of headings within fiction
            ...
            if (volume1983_and_later==1): # deal with volumes after 1983 individually
                if((volume1983==1 and text[j] == "1642") or (volume1984==1 and text[j]=="1766") or (volume1985==1 and text[j]=="1816") or (volume1986==1 and text[j]=="1841") or (volume1987==1 and text[j]=="2130") or (volume1988==1 and text[j]=="1974") or (volume1989==1 and text[j]=="1909")):
                    for k in range(1,linelength-j):
                        if(text[j+k]=='Fiction'):
                ...
                for k in list(range(1, linelength - j)):
                    ...
                    if ((volume1976_and_later==0 and len(text[j + k]) > 1 and len(text[k + j]) <= 30 and '(' not in text[
                        ...
                        if (text[k + j][0] >= 'A' and text[k + j][0] <= 'Z' and text[k + j][1] >= 'a' and
                                ...
                                k + j] not in wrong_headings and text [k+j][:14]!='Fiction themes' and text [k+j][-11:]!='Collections' and text[k+j][0:13]!='United States' and text[k+j][0:13]!='United State*' and text[k+j][0:13]!='United Btates' and text[k+j][0:13]!='United Slutcs' and text[k+j][0:12]!='United 8tate' and text[k+j][-13:]!='United States' and text[k+j][0:6]!='France' and text[k+j][0:6]!='Russia' and text[k+j][0:7]!='America' and text[k+j][0:7]!='England' and text[k+j][0:7]!='Britain' and text[k+j][0:13]!='Great Britain' and text[k+j][0:13]!='Qreat Britain' and text[k+j][0:7]!='Germany' and text[k+j][0:7]!='Austria' and text[k+j][0:11]!='Set Fiction' and text[k+j][0:11]!='Bee Fiction' and text[k+j][0:26]!='Swados, H. Story for Teddy'):  # For wrong headings, ignore them; first character A-Z, second a-z (avoiding "BOOK REVIEW DIGEST"), last a-z or 0-9 (eg "Adams, Henry, 1838-1918")
                            if (len(text[
