...
for (i in 1934:2006) {
  thesediffs = data$fullrandomdiff[data$meandate >= i - 3 & data$meandate <= i + 3]
...
p <- ggplot(df, aes(x = meandate, y = diff)) + 
  ...
  theme_bw() + ggtitle('Difference between in-genre and random comparisons (Kirkus)') +
  scale_x_continuous("") +
...
tiff("kirkusfullrandomdiffs.tiff", height = 6, width = 9, units = 'in', res=400)
plot(p)
dev.off()
plot(p)